...
export function buildTailorPrompt({ profileJson, jdText, steering, gapAnswers, keywordList }) {
  ...
  const baseRules = hasGapContext
    ? [
      ...
      '',
      'BULLET FORMULA',
      '- Rewrite every bullet using this formula: Accomplished X, measured by Y (a real number if one exists), by doing Z (the method/tool/approach).',
    ...
    : [
      ...
      '(2) Bullet Formula: Accomplished X, measured by Y, by doing Z. Every bullet should solve a specific problem.',
