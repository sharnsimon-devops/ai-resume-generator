...
export function buildTailorPrompt({ profileJson, jdText, steering, gapAnswers, keywordList }) {
  ...
  const baseRules = hasGapContext
    ? [
      ...
      '',
      'KEYWORD MIRRORING & REPHRASING',
      "- Aggressively rephrase the candidate's experience using the exact terminology and keywords from the JD.",
      "- It is completely acceptable and expected to translate the candidate's existing experience into the JD's preferred terms (e.g., if they did 'backend APIs', you can call it 'microservices' if the JD asks for it).",
      '',
      'HONESTY — HARD CONSTRAINTS',
      "- Do not invent completely fake employers, titles, dates, or entirely new disciplines (e.g., don't invent nursing experience for a software engineer).",
      "- You MAY adopt tools and skill keywords from the JD if they are reasonable synonyms or closely related to what the candidate has done.",
      "- Where a gap was flagged and the user's answer only partially confirms the requirement, phrase the bullet exactly as caveated by their answer.",
      '',
